...
const AuthDemoOptions: React.FC<AuthDemoOptionsProps> = ({
  ...
  return <div className="mt-6 pt-6 border-t border-muted">
      <p className="text-center text-sm mb-4">
        Try the <span className="font-semibold">demo version</span> instead
      </p>
      
      <div className="flex flex-col space-y-3">
        <Button onClick={handleDemoLogin} variant="outline" className="w-full py-2 border border-border hover:bg-accent text-sm font-medium transition-colors">
          Fill in Demo Credentials
        </Button>
        
        <Button onClick={handleDirectDemoLogin} variant="default" className="w-full py-2 bg-salesBlue hover:bg-salesBlue/90 text-sm font-medium transition-colors text-indigo-700">
          Quick Demo Login
        </Button>
      </div>
