  ...
  return <Card className={`w-full ${isFullUser ? 'border-2 border-gradient-to-r from-blue-500 to-purple-600' : ''}`}>
      <CardHeader className={`${isFullUser ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white' : 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white'} rounded-t-lg`}>
        <CardTitle className="flex items-center justify-between">
          <span className="text-lg font-semibold text-slate-50">AI Daily Summary</span>
          <Button variant="ghost" size="sm" onClick={playAudio} disabled={isPlaying} className="text-white hover:bg-white/10">
      ...
      <CardContent className="p-6">
        <div className="space-y-3">
          ...
          {validSummary.length > 150 && <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="text-primary hover:text-primary/80 p-0 h-auto font-medium">
              {isExpanded ? <>
          ...
          {isPlaying && <div className="flex items-center gap-2 text-sm text-blue-600">
