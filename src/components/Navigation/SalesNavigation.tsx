  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-background border-b border-border shadow-sm">
      <div className="h-[60px] flex items-center justify-between px-6">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          ...
          const isActive = location.pathname === item.href || item.href.includes('dashboard') && location.pathname === '/sales' || item.href.includes('lead-management') && location.pathname.includes('lead-management');
        ...
        <div className="flex items-center space-x-4">
          <ThemeToggle />
          <UserProfile name={profile?.full_name || "Sales Rep"} role="Sales Representative" />
        </div>
      </div>
    </header>;
};
